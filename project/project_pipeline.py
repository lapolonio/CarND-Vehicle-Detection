...
def image_processor(image):
    ...
    scaled_box_params = [
        dict(scale=3.0, y_start=[400, 700]),
        dict(scale=2.5, y_start=[400, 650]),
        dict(scale=2.0, y_start=[400, 600]),
        dict(scale=1.5, y_start=[400, 550]),
    ...
    draw_img = draw_labeled_bboxes(np.copy(image).astype(np.float32)*255, labels)

    return draw_img

project_output_name = '../test_output/sample_output_project_video.mp4'
# clip2 = VideoFileClip('../test_video.mp4')
clip2 = VideoFileClip('../project_video.mp4')
# project_clip = clip2.subclip(1, 4).fl_image(image_processor)
